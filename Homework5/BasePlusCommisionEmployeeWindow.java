...
public class BasePlusCommisionEmployeeWindow extends JDialog{
	private BasePlusCommissionEmployee bpce;
	
	public BasePlusCommisionEmployeeWindow() {
		...
		// Base Salary
		JLabel bsLabel = new JLabel("Base Salary:");
		bsLabel.setBounds(10, 170, 150, 25);
		panel.add(bsLabel);

		JTextField bsText = new JTextField(20);
		bsText.setBounds(150, 170, 265, 25);
		panel.add(bsText);

		// Submit Button
		JButton submitButton = new JButton("Submit");
		submitButton.setBounds(175, 200, 100, 25);
		...
		ActionListener al = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					bpce = new BasePlusCommissionEmployee(firstNameText.getText(), lastNameText.getText(), Integer.parseInt(ssnText.getText()));
					bpce.setGrossSales(Integer.parseInt(gsText.getText()));
					bpce.setCommissionRate(Double.parseDouble(crText.getText()));
					bpce.setBaseSalary(Double.parseDouble(bsText.getText()));
					dial.dispose();
				} catch (Exception exc) {
					// Exception
